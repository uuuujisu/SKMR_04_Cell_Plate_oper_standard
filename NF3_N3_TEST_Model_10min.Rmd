...
df_1h_10min <- df2 %>%
  ...
  ungroup
df_1h_10min <- df_1h_10min[,c(1:4,38:98)] %>% arrange(File_num,Item_No) %>% distinct()

df_1h_10min[sapply(df_1h_10min,is.infinite)] <- NA
df_1h_10min[sapply(df_1h_10min,is.nan)] <- NA
NArow <- which(is.na(df_1h_10min),arr.ind=TRUE)
df_1h_10min <- df_1h_10min[-NArow[,1],]

df_1h_10min$y <- as.factor(df_1h_10min$y)

```

### Data Importance & Model
```{r rf modeling , message=FALSE, echo=FALSE, warning=FALSE }
set.seed(0713)

# 전류 load high
load_high <- df_1h_10min %>% filter(전류load_1h >= 9500)
DF <- load_high[,-c(1:3)]
# importance
forest <- randomForest(y~., data=DF)
plot_1h <- varImpPlot(forest)
# 전체 변수
my.control<-rpart.control(xval=10, minsplit=15)
