...
Merge_df_tmp <- Merge_df %>% filter(Item_No ==aa)
...
for (loop in File_Num) {
  ...
  tmp_df$연소발생일 <- as.POSIXct(tmp_df$연소발생일, origin="1899-12-30", tz="GMT")
 ...
 # print(p)
  ...
  tmp_df <- merge(tmp_df1, tmp_df2, by = c("File_num","Item_No","연소여부","Time","전류","Plates전류합",
                                           "액보충시간","연소발생일","GROUP_endtime","GROUP_starttime", "Num")) %>% 
...
for (aa in Explosion_Cell_total[c(13,24)]){
 ...
 #   tmp_df$액보충시간 <- as.POSIXct(tmp_df$액보충시간, origin="1899-12-30", tz="GMT")
  ...
  #    geom_text( aes(x = 액보충시간, y = 0, label = c("액보충시간")))+
    ...
    tmp_df <- merge(tmp_df1, tmp_df2, by = c("File_num","Item_No","연소여부","Time","전류","Plates전류합",
                                             "연소발생일","GROUP_endtime","GROUP_starttime", "Num")) %>% 
      arrange(Time);#rm(tmp_df1,tmp_df2)
