...
for(i in 1:length(fileList)){
    ...
    # kable(Explotion_Data_df, "html") %>% kable_styling(full_width = F)
}   
...
# N3 연소발생여부 data에서 J-V cell Monel Type filtering
N3_Explosion_DF_tmp <- N3_Explosion_DF %>% 
    filter(Cell종류=="Monel"&Item_No=="J-V")
...
File_Num <- unique(Merge_df$File_num)

for (loop in 1:length(File_Num)) {
    tmp_df <- Merge_df %>% filter(File_num==loop)
